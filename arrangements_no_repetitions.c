/*Порождение размещений без повторений. C(89). MSK. dcc0yandex.ru*/
#include <stdio.h>
#include <stdlib.h>
...
int permute(int* arg) {
  ...
  for (i = 0; i < x; i++) {
    for (j = x - 1; j > i; j--) {
      ...
      /*Если нашли дубликат, то выйдем из функции*/
      if (argv[j - 1] == argv[j]) {
        return 0;
      }
    }
...
int main(int argc, char* argv[]) {
  ...
  if (n < k) {
    printf("N must be greater than k. Example: 8 4\n");
    return 0;
  }
    ...
    if (b[k - 1] == n) {
      ...
      while (i >= -1) {
        ...
        if (more_per_unit != -1) {
           ...
           *Here we transfer elements to C and fill array B till K.*/
          for (j = i; j < k - 1; j++) {
